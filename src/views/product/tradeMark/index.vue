 ...
 * @Description: 
 * @Author: RyangXie
 * @Date: 2022-01-03 20:47:37
 * @LastEditTime: 2022-01-04 23:55:00
...
<template>
  <div>
    ...
    <!-- 对话框 -->
    <el-dialog :title="this.tmForm.id?'修改品牌':'添加品牌'"
    ...
    addOrUpdateTradeMark () {
      ...
      this.$refs.ruleForm.validate(async (success) => {
        if (success) {
          ...
          if (result.code == 200) {
            // 弹出信息
            this.$message.success(this.tmForm.id ? '修改品牌成功' : '添加产品成功')
            // 如果添加品牌，停留在第一页，修改品牌应该在当前页
            this.getPageList(this.tmForm.id ? this.page : 1)
          }
        } else {
          this.$message.error(this.tmForm.id ? '修改品牌失败' : '添加产品失败')
        }
      })
    },
    // 删除品牌的操作
